...
function replacePrivateKey() {
  ...
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/IIT_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose.yml
  cd crypto-config/peerOrganizations/mhrd.certification-network.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/MHRD_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose.yml
  cd crypto-config/peerOrganizations/upgrad.certification-network.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/UPGRAD_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose.yml
